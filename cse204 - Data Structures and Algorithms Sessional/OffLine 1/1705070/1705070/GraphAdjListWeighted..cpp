...
void Graph::removeEdge(int u, int v) {
    ...
    if (!directed) {
        ...
        if (t != NULL_VALUE && x != NULL_VALUE) {
            ...
            weightList[u].removeItemAt(t);
            weightList[v].removeItemAt(x);
        }
    } else {
        int t = adjList[u].searchItem(v);
        if (t != NULL_VALUE) {
...
void Graph::bfs(int source) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < adjList[source].getLength(); i++) {
            int s=adjList[source].getItem(i),d=weightList[source].getItem(i);
            if (!visited[s] && color[s] == WHITE) {
                visited[s] = true;
                color[s] = GREY;
                countDistance[s] = countDistance[source] + d;
                q.enqueue(s);
            }
        }
        color[source] = BLACK;
...
void Graph::BellmanFord(int source){
    for (int i=0;i < nVertices -1;i++){
        for(int j=0;j<nEdges-2;j++){
            int f1 = weightList[source].getItem(j);
            int f3 = weightList[source].getItem(j+2);
            int f2 = weightList[source].getItem(j+1);
            if( f1+f2 < f3 ){
                countDistance[j] = f1+f2;
            }
        }
    }
}
